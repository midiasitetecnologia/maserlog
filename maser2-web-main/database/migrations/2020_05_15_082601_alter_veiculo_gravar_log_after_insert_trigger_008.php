...
class AlterVeiculoGravarLogAfterInsertTrigger008 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_insert AFTER INSERT ON veiculo FOR EACH ROW
                BEGIN                                    
					...
					IF (IFNULL(NEW.transfer_code, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "insert", "placa", NEW.placa, @client_addr, "transfer_code", 
                          NEW.transfer_code
                        ); 
					END IF;
					
					IF (IFNULL(NEW.dt_transfer_code, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "insert", "placa", NEW.placa, @client_addr, "dt_transfer_code", 
                          NEW.dt_transfer_code
                        ); 
                    END IF;
