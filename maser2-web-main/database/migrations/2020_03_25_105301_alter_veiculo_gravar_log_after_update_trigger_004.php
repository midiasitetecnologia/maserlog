...
class AlterVeiculoGravarLogAfterUpdateTrigger004 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_update AFTER UPDATE ON veiculo FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.ignicao, "") != IFNULL(NEW.ignicao, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "update", "placa", NEW.placa, @client_addr, "ignicao", 
                          OLD.ignicao, NEW.ignicao
                        ); 
                    END IF;
