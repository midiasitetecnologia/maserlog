	...
	<div class="vx-row">
		<div class="vx-col w-full">
			<vx-card>
				<vs-table
					ref="table"
					:noDataText="entregasAndamentoData.length > 0 ? 'Não há registros para mostrar. Verifique o texto pesquisado e a página atual.' : 'Não há registros para mostrar.'"
					v-model="selected"
					search
					:max-items="entregasAndamentoData.length"
					stripe
					:data="entregasAndamentoData"
				>
					<div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
						<div class="flex mb-1">
							<span class="font-semibold">Entregas Andamento</span>
						</div>
							...
							<vs-td :data="data[indextr].local_coleta">
								<div>
									<span
										...
										@click="fillEntregasAndamento(data[indextr]);
		...
		<vs-popup class="holamundo" :title="tituloPopup" :active.sync="popupActive">
			...
			<template v-if="(funcaoPopup == 'coleta')">
				<p
					v-if="dadosEntregasAnd.dt_efet_coleta != null || dadosEntregasAnd.hr_partida_coleta != null || dadosEntregasAnd.hr_cheg_coleta != null || dadosEntregasAnd.hr_atend_coleta != null || dadosEntregasAnd.hr_sai_coleta != null"
					class="mt-4"
				>
					<template>
						<div v-if="dadosEntregasAnd.dt_efet_coleta != null">
							<span class="mr-1 font-semibold">Data da coleta:</span>
							{{dadosEntregasAnd.dt_efet_coleta | moment("DD MMM YYYY")}}
						</div>
						<div>
							<span v-if="dadosEntregasAnd.hr_partida_coleta != null" class="mr-4">
								<span class="mr-1 font-semibold">Partida:</span>
								{{dadosEntregasAnd.hr_partida_coleta | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_cheg_coleta != null" class="mr-4">
								<span class="mr-1 font-semibold">Chegada:</span>
								{{dadosEntregasAnd.hr_cheg_coleta | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_atend_coleta != null" class="mr-4">
								<span class="mr-1 font-semibold">Atendimento:</span>
								{{dadosEntregasAnd.hr_atend_coleta | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_sai_coleta != null" class="mr-4">
								<span class="mr-1 font-semibold">Saída:</span>
								{{dadosEntregasAnd.hr_sai_coleta | hora_min}}
							</span>
						</div>
					</template>
				</p>
				...
				<p
					v-if="dadosEntregasAnd.placa_coleta != null || dadosEntregasAnd.nome_motorista_coleta != null"
					class="mt-4"
				>
					<template>
						<div v-if="dadosEntregasAnd.placa_coleta != null">
							<span class="mr-1 font-semibold">Veículo:</span>
							{{dadosEntregasAnd.placa_coleta}}
						</div>
						<div v-if="dadosEntregasAnd.nome_motorista_coleta != null">
							<span class="mr-1 font-semibold">Motorista:</span>
							{{dadosEntregasAnd.nome_motorista_coleta}}
						</div>
					</template>
				</p>
			...
			<template v-if="(funcaoPopup == 'entrega')">
				<p
					v-if="dadosEntregasAnd.dt_efet_entrega != null || dadosEntregasAnd.hr_partida_entrega != null || dadosEntregasAnd.hr_cheg_entrega != null || dadosEntregasAnd.hr_atend_entrega != null || dadosEntregasAnd.hr_sai_entrega != null"
					class="mt-4"
				>
					<template>
						<div v-if="dadosEntregasAnd.dt_efet_entrega != null">
							<span class="mr-1 font-semibold">Data da entrega:</span>
							{{dadosEntregasAnd.dt_efet_entrega | moment("DD MMM YYYY")}}
						</div>
						<div>
							<span v-if="dadosEntregasAnd.hr_partida_entrega != null" class="mr-4">
								<span class="mr-1 font-semibold">Partida:</span>
								{{dadosEntregasAnd.hr_partida_entrega | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_cheg_entrega != null" class="mr-4">
								<span class="mr-1 font-semibold">Chegada:</span>
								{{dadosEntregasAnd.hr_cheg_entrega | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_atend_entrega != null" class="mr-4">
								<span class="mr-1 font-semibold">Atendimento:</span>
								{{dadosEntregasAnd.hr_atend_entrega | hora_min}}
							</span>
							<span v-if="dadosEntregasAnd.hr_sai_entrega != null" class="mr-4">
								<span class="mr-1 font-semibold">Saída:</span>
								{{dadosEntregasAnd.hr_sai_entrega | hora_min}}
							</span>
						</div>
					</template>
				</p>
				...
				<p
					v-if="dadosEntregasAnd.placa_entrega != null || dadosEntregasAnd.nome_motorista_entrega != null"
					class="mt-4"
				>
					<template>
						<div v-if="dadosEntregasAnd.placa_entrega != null">
							<span class="mr-1 font-semibold">Veículo:</span>
							{{dadosEntregasAnd.placa_entrega}}
						</div>
						<div v-if="dadosEntregasAnd.nome_motorista_entrega != null">
							<span class="mr-1 font-semibold">Motorista:</span>
							{{dadosEntregasAnd.nome_motorista_entrega}}
						</div>
					</template>
				</p>

				<p v-if="dadosEntregasAnd.recebedor != null" class="mt-4">
					<span class="mr-1 font-semibold">Recebedor:</span>
					{{dadosEntregasAnd.recebedor}}
				</p>
			</template>
		</vs-popup>
...
export default {
	...
	data() {
		return {
			...
			//Dados da Coleta - Entregas Andamento.
			dadosEntregasAnd: [],
	...
	methods: {
		...
		fillEntregasAndamento(entregasAndamento) {
			this.dadosEntregasAnd = entregasAndamento;
		},
		exibirDados(
			funcao,
			dadosColeta,
