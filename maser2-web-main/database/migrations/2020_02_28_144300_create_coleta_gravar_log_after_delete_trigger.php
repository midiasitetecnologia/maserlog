...
class CreateColetaGravarLogAfterDeleteTrigger extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_gravar_log_after_delete AFTER DELETE ON coleta FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.tipo_carga, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "tipo_carga", OLD.tipo_carga
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.km_inicial, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "km_inicial", OLD.km_inicial
                        );
                    END IF;

                    IF (IFNULL(OLD.km_final, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "km_final", OLD.km_final
                        );
                    END IF;

                    IF (IFNULL(OLD.geo_lat_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "geo_lat_coleta", OLD.geo_lat_coleta
                        );
                    END IF;

                    IF (IFNULL(OLD.geo_lng_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "geo_lng_coleta", OLD.geo_lng_coleta
                        );
                    END IF;

                    IF (IFNULL(OLD.geo_lat_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "geo_lat_entrega", OLD.geo_lat_entrega
                        );
                    END IF;

                    IF (IFNULL(OLD.geo_lng_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "geo_lng_entrega", OLD.geo_lng_entrega
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.descarga_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "descarga_coleta", OLD.descarga_coleta
                        );
                    END IF;

                    IF (IFNULL(OLD.dt_descarga_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "dt_descarga_coleta", OLD.dt_descarga_coleta
                        );
                    END IF;
