...
class AlterColetaGravarLogAfterDeleteTrigger014 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_gravar_log_after_delete AFTER DELETE ON coleta FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.hr_partida_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "hr_partida_coleta", OLD.hr_partida_coleta
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.hr_partida_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "hr_partida_entrega", OLD.hr_partida_entrega
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.tempo_desloc_pavilhao, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "tempo_desloc_pavilhao", OLD.tempo_desloc_pavilhao
                        );
                    END IF;

                    IF (IFNULL(OLD.entrega_consolidada, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "entrega_consolidada", OLD.entrega_consolidada
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.aceitar_foto_rom, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "aceitar_foto_rom", OLD.aceitar_foto_rom
                        );
                    END IF;

                    IF (IFNULL(OLD.ocultar_resumo, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "ocultar_resumo", OLD.ocultar_resumo
                        );
                    END IF;

                    IF (IFNULL(OLD.tipo_frete, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "tipo_frete", OLD.tipo_frete
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.carga_pavilhao, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "carga_pavilhao", OLD.carga_pavilhao
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.mot_nao_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "mot_nao_entrega", OLD.mot_nao_entrega
                        );
                    END IF;

                    IF (IFNULL(OLD.obs_nao_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "obs_nao_entrega", OLD.obs_nao_entrega
                        );
                    END IF;

                    IF (IFNULL(OLD.reentrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "reentrega", OLD.reentrega
                        );
                    END IF;

                    IF (IFNULL(OLD.reentrega_gerada, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "reentrega_gerada", OLD.reentrega_gerada
                        );
                    END IF;

                    IF (IFNULL(OLD.solic_reentrega_id, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta", "delete", "id", OLD.id, @client_addr, "solic_reentrega_id", OLD.solic_reentrega_id
                        );
                    END IF;
