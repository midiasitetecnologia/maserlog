<template>
	<div>
		<div v-if="$acl.check('admin')" class="flex flex-wrap-reverse items-center data-list-btn-container">
			<div 
		...
		<div class="vx-row">
			<div class="vx-col w-full">
				<vx-card>
					<vs-table
						ref="table"
						:noDataText="usersData.length > 0 ? 'Não há registros para mostrar. Verifique o texto pesquisado e a página atual.' : 'Não há registros para mostrar.'"
						v-model="selected"
					...
					>
							...
							<div class="flex flex-wrap-reverse items-center">
								...
								<vs-dropdown vs-trigger-click class="cursor-pointer mr-4 items-per-page-handler">
									...
									>
										<span
											class
										>{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ usersData.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : usersData.length }} de {{ queriedItems }}</span>
						...
						<template slot="thead">
							...
							<vs-th sort-key="email">Email / ID Login</vs-th>
							<vs-th sort-key="user_type">Tipo de Usuário</vs-th>
						...
						<template slot-scope="{data}">
							<vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
								...
								<vs-td :data="data[indextr].user_type">{{ data[indextr].user_type | userTypeLabel }}</vs-td>
								...
								<vs-td class="whitespace-no-wrap">
									...
									/>
									<feather-icon
										v-if="$acl.check('admin') && ($store.state.AppActiveUser.uid != data[indextr].id)"										
										icon="TrashIcon"
										svgClasses="w-5 h-5 hover:text-danger stroke-current cursor-pointer"
										class="ml-2"
										@click="confirmDeleteRecord(tr.id, tr.name)"
									/>
								</vs-td>
								<vs-td class="whitespace-no-wrap">
									<vs-button type="border" size="small" class="mr-2" @click="alterarSenha(tr.id)">Alterar Senha</vs-button>
								</vs-td>
							</vs-tr>
						</template>
					</vs-table>
...
export default {
	...
	methods: {
		async getUsers() {
			await this.$store.dispatch("users/indexUsers").catch(err => {
				console.error(err);
			});
		},
		alterarSenha(id) {
			this.$router.push("/users_alterar_senha/" + id).catch(() => {});
		},
		editRecord(id) {
			this.$router.push("/users/" + id + "/edit").catch(() => {});
		},
		addNewData() {
			this.$router.push("/users/create/").catch(() => {});
		},
		confirmDeleteRecord(id, name) {
			this.$vs.dialog({
				...
				text: `Atenção! Esta operação é definitiva. Deseja realmente excluir este usuário "${id} - ${name}"?`,
