...
class AlterVeiculoGravarLogAfterDeleteTrigger008 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_delete AFTER DELETE ON veiculo FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.nivel_cons, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "veiculo", "delete", "placa", OLD.placa, @client_addr, "nivel_cons", OLD.nivel_cons
                        ); 
                    END IF;
                    ...
                    IF (IFNULL(OLD.placa_cavalo, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "veiculo", "delete", "placa", OLD.placa, @client_addr, "placa_cavalo", OLD.placa_cavalo
                        ); 
                    END IF;

                    IF (IFNULL(OLD.transfer_code, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "veiculo", "delete", "placa", OLD.placa, @client_addr, "transfer_code", OLD.transfer_code
                        ); 
                    END IF;

                    IF (IFNULL(OLD.dt_transfer_code, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "veiculo", "delete", "placa", OLD.placa, @client_addr, "dt_transfer_code", OLD.dt_transfer_code
                        ); 
                    END IF;
