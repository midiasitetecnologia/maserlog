...
class AlterMotoristaGravarLogAfterDeleteTrigger003 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_motorista_gravar_log_after_delete AFTER DELETE ON motorista FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.hr_ini_exped, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "hr_ini_exped", OLD.hr_ini_exped
                        ); 
                    END IF;

                    IF (IFNULL(OLD.hr_fim_exped, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "hr_fim_exped", OLD.hr_fim_exped
                        ); 
                    END IF;

                    IF (IFNULL(OLD.hr_ini_login, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "hr_ini_login", OLD.hr_ini_login
                        ); 
                    END IF;

                    IF (IFNULL(OLD.hr_fim_login, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "hr_fim_login", OLD.hr_fim_login
                        ); 
                    END IF;

                    IF (IFNULL(OLD.auto_logoff, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "auto_logoff", OLD.auto_logoff
                        ); 
                    END IF;
                    ...
                    IF (IFNULL(OLD.dt_alt_cad, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "dt_alt_cad", OLD.dt_alt_cad
                        ); 
                    END IF;
                    ...
                    IF (IFNULL(OLD.hr_alt_cad, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "motorista", "delete", "id", OLD.id, @client_addr, "hr_alt_cad", OLD.hr_alt_cad
                        ); 
                    END IF;
