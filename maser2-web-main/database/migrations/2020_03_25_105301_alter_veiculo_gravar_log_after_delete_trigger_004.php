...
class AlterVeiculoGravarLogAfterDeleteTrigger004 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_delete AFTER DELETE ON veiculo FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.ignicao, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "veiculo", "delete", "placa", OLD.placa, @client_addr, "ignicao", OLD.ignicao
                        ); 
                    END IF;
