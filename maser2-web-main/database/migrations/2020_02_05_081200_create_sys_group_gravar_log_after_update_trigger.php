...
class CreateSysGroupGravarLogAfterUpdateTrigger extends Migration
{
    /**
     *
    ...
    public function up()
    {
        DB::unprepared('DROP TRIGGER IF EXISTS tr_sys_group_gravar_log_after_update');
        DB::unprepared('
            CREATE TRIGGER tr_sys_group_gravar_log_after_update AFTER UPDATE ON sys_group FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.name, "") != IFNULL(NEW.name, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_group", "update", "id", NEW.id, @client_addr, "name", OLD.name, 
                          NEW.name
                        );                    
                    END IF;
