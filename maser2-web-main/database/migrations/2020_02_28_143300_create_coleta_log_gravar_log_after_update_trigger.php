...
class CreateColetaLogGravarLogAfterUpdateTrigger extends Migration
{
    /**
     *
    ...
    public function up()
    {
        DB::unprepared('DROP TRIGGER IF EXISTS tr_coleta_log_gravar_log_after_update');
        DB::unprepared('
            CREATE TRIGGER tr_coleta_log_gravar_log_after_update AFTER UPDATE ON coleta_log FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.coleta_id, "") != IFNULL(NEW.coleta_id, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_log", "update", "id", NEW.id, @client_addr, "coleta_id", OLD.coleta_id, 
                          NEW.coleta_id
                        );                    
                    END IF;

                    IF (IFNULL(OLD.tipo, "") != IFNULL(NEW.tipo, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_log", "update", "id", NEW.id, @client_addr, "tipo", OLD.tipo, 
                          NEW.tipo
                        );
                    END IF;                    
                    ...
                    IF (IFNULL(OLD.descricao, "") != IFNULL(NEW.descricao, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_log", "update", "id", NEW.id, @client_addr, "descricao", 
                          OLD.descricao, NEW.descricao
                        ); 
                    END IF;

                    IF (IFNULL(OLD.texto, "") != IFNULL(NEW.texto, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value_blob, new_value_blob
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_log", "update", "id", NEW.id, @client_addr, "texto", 
                          OLD.texto, NEW.texto
                        ); 
                    END IF;
