...
class AlterVeiculoGravarLogAfterUpdateTrigger008 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_update AFTER UPDATE ON veiculo FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.nivel_cons, "") != IFNULL(NEW.nivel_cons, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "update", "placa", NEW.placa, @client_addr, "nivel_cons", 
                          OLD.nivel_cons, NEW.nivel_cons
                        ); 
                    END IF;
					...
					IF (IFNULL(OLD.placa_cavalo, "") != IFNULL(NEW.placa_cavalo, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "update", "placa", NEW.placa, @client_addr, "placa_cavalo", 
                          OLD.placa_cavalo, NEW.placa_cavalo
                        ); 
					END IF;
					
					IF (IFNULL(OLD.transfer_code, "") != IFNULL(NEW.transfer_code, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "update", "placa", NEW.placa, @client_addr, "transfer_code", 
                          OLD.transfer_code, NEW.transfer_code
                        ); 
					END IF;
					
					IF (IFNULL(OLD.dt_transfer_code, "") != IFNULL(NEW.dt_transfer_code, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "update", "placa", NEW.placa, @client_addr, "dt_transfer_code", 
                          OLD.dt_transfer_code, NEW.dt_transfer_code
                        ); 
                    END IF;
