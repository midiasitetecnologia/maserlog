...
class AlterColetaNfGravarLogAfterInsertTrigger004 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_nf_gravar_log_after_insert AFTER INSERT ON coleta_nf FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.solic_distrib_id, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "insert", "id", NEW.id, @client_addr, "solic_distrib_id", NEW.solic_distrib_id
                        );                    
                    END IF;
