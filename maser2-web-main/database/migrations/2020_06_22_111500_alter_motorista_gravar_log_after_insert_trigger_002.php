...
class AlterMotoristaGravarLogAfterInsertTrigger002 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_motorista_gravar_log_after_insert AFTER INSERT ON motorista FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.hr_ini_login, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "insert", "id", NEW.id, @client_addr, "hr_ini_login", 
                          NEW.hr_ini_login
                        ); 
                    END IF;

                    IF (IFNULL(NEW.hr_fim_login, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "insert", "id", NEW.id, @client_addr, "hr_fim_login", 
                          NEW.hr_fim_login
                        ); 
                    END IF;

                    IF (IFNULL(NEW.auto_logoff, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "insert", "id", NEW.id, @client_addr, "auto_logoff", 
                          NEW.auto_logoff
                        ); 
                    END IF;
					...
					IF (IFNULL(NEW.dt_alt_cad, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "insert", "id", NEW.id, @client_addr, "dt_alt_cad", 
                          NEW.dt_alt_cad
                        ); 
                    END IF;
                    ...
                    IF (IFNULL(NEW.hr_alt_cad, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "insert", "id", NEW.id, @client_addr, "hr_alt_cad", 
                          NEW.hr_alt_cad
                        ); 
                    END IF;
