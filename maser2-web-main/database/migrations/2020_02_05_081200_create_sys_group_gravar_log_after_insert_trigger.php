...
class CreateSysGroupGravarLogAfterInsertTrigger extends Migration
{
    /**
     *
    ...
    public function up()
    {   
        DB::unprepared('DROP TRIGGER IF EXISTS tr_sys_group_gravar_log_after_insert');
        DB::unprepared('
            CREATE TRIGGER tr_sys_group_gravar_log_after_insert AFTER INSERT ON sys_group FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.name, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_group", "insert", "id", NEW.id, @client_addr, "name", NEW.name
                        );                    
                    END IF;
