...
class AlterSysCfgGravarLogAfterInsertTrigger004 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_sys_cfg_gravar_log_after_insert AFTER INSERT ON sys_cfg FOR EACH ROW
                BEGIN                                    
					...
					IF (IFNULL(NEW.gerar_coletas_fixas, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "insert", "id", NEW.id, @client_addr, "gerar_coletas_fixas", 
                          NEW.gerar_coletas_fixas
                        ); 
                    END IF;

					IF (IFNULL(NEW.dia_coletas_fixas, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "insert", "id", NEW.id, @client_addr, "dia_coletas_fixas", 
                          NEW.dia_coletas_fixas
                        ); 
                    END IF;
