...
class AlterColetaNfGravarLogAfterUpdateTrigger004 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_coleta_nf_gravar_log_after_update AFTER UPDATE ON coleta_nf FOR EACH ROW
                BEGIN
					...
					IF (IFNULL(OLD.solic_distrib_id, "") != IFNULL(NEW.solic_distrib_id, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "solic_distrib_id", OLD.solic_distrib_id, 
                          NEW.solic_distrib_id
                        );                    
					END IF;
