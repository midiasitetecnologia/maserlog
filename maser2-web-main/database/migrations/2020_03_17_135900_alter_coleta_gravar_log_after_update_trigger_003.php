...
class AlterColetaGravarLogAfterUpdateTrigger003 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_gravar_log_after_update AFTER UPDATE ON coleta FOR EACH ROW
                BEGIN
					...
					IF (IFNULL(OLD.km_ini_entrega, "") != IFNULL(NEW.km_ini_entrega, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "km_ini_entrega", OLD.km_ini_entrega, 
                          NEW.km_ini_entrega
                        );                    
					END IF;
					
					IF (IFNULL(OLD.km_fim_entrega, "") != IFNULL(NEW.km_fim_entrega, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "km_fim_entrega", OLD.km_fim_entrega, 
                          NEW.km_fim_entrega
                        );                    
                    END IF;
