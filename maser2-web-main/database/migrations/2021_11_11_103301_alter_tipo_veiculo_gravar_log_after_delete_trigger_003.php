...
class AlterTipoVeiculoGravarLogAfterDeleteTrigger003 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_tipo_veiculo_gravar_log_after_delete AFTER DELETE ON tipo_veiculo FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.dur_prev_atend, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "tipo_veiculo", "delete", "codigo", OLD.codigo, @client_addr, "dur_prev_atend", OLD.dur_prev_atend
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.tempo_desloc_pavilhao, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "tipo_veiculo", "delete", "codigo", OLD.codigo, @client_addr, "tempo_desloc_pavilhao", OLD.tempo_desloc_pavilhao
                        );
                    END IF;
