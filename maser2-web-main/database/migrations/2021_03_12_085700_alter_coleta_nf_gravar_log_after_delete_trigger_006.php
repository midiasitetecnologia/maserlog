...
class AlterColetaNfGravarLogAfterDeleteTrigger006 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_nf_gravar_log_after_delete AFTER DELETE ON coleta_nf FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.dig_cnpj, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta_nf", "delete", "id", OLD.id, @client_addr, "dig_cnpj", OLD.dig_cnpj
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.solic_destino_id, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta_nf", "delete", "id", OLD.id, @client_addr, "solic_destino_id", OLD.solic_destino_id
                        );
                    END IF;

                    IF (IFNULL(OLD.substituida, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta_nf", "delete", "id", OLD.id, @client_addr, "substituida", OLD.substituida
                        );
                    END IF;
                    ...
                    IF (IFNULL(OLD.observ, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "coleta_nf", "delete", "id", OLD.id, @client_addr, "observ", OLD.observ
                        );
                    END IF;
