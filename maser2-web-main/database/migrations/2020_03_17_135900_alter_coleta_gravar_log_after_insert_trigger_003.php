...
class AlterColetaGravarLogAfterInsertTrigger003 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_coleta_gravar_log_after_insert AFTER INSERT ON coleta FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.dt_desloca_coleta, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "insert", "id", NEW.id, @client_addr, "dt_desloca_coleta", NEW.dt_desloca_coleta
                        );                    
                    END IF;
                    ...
                    IF (IFNULL(NEW.dt_desloca_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "insert", "id", NEW.id, @client_addr, "dt_desloca_entrega", NEW.dt_desloca_entrega
                        );                    
                    END IF;

                    IF (IFNULL(NEW.km_ini_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "insert", "id", NEW.id, @client_addr, "km_ini_entrega", NEW.km_ini_entrega
                        );                    
                    END IF;

                    IF (IFNULL(NEW.km_fim_entrega, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "insert", "id", NEW.id, @client_addr, "km_fim_entrega", NEW.km_fim_entrega
                        );                    
                    END IF;
