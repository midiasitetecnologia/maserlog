...
class AlterClienteGravarLogAfterInsertTrigger002 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_cliente_gravar_log_after_insert AFTER INSERT ON cliente FOR EACH ROW
                BEGIN                                    
					...
					IF (IFNULL(NEW.solicitar_coletas, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "cliente", "insert", "id", NEW.id, @client_addr, "solicitar_coletas", 
                          NEW.solicitar_coletas
                        ); 
                    END IF;
