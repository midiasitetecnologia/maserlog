...
class CreateUsersGravarLogAfterInsertTrigger extends Migration
{
    /**
     *
            ...
            CREATE TRIGGER tr_users_gravar_log_after_insert AFTER INSERT ON users FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.user_type, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "users", "insert", "id", NEW.id, @client_addr, "user_type", 
                          NEW.user_type
                        ); 
                    END IF;

                    IF (IFNULL(NEW.cliente_id, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "users", "insert", "id", NEW.id, @client_addr, "cliente_id", 
                          NEW.cliente_id
                        ); 
                    END IF;
