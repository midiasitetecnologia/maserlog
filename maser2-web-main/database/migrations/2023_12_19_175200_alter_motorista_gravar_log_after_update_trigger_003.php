...
class AlterMotoristaGravarLogAfterUpdateTrigger003 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_motorista_gravar_log_after_update AFTER UPDATE ON motorista FOR EACH ROW
                BEGIN
					...
					IF (IFNULL(OLD.hr_ini_exped, "") != IFNULL(NEW.hr_ini_exped, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "update", "id", NEW.id, @client_addr, "hr_ini_exped", 
                          OLD.hr_ini_exped, NEW.hr_ini_exped
                        ); 
					END IF;
					
					IF (IFNULL(OLD.hr_fim_exped, "") != IFNULL(NEW.hr_fim_exped, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "motorista", "update", "id", NEW.id, @client_addr, "hr_fim_exped", 
                          OLD.hr_fim_exped, NEW.hr_fim_exped
                        ); 
					END IF;
