...
class CreateSysUserGroupGravarLogAfterUpdateTrigger extends Migration
{
    /**
     *
    ...
    public function up()
    {
        DB::unprepared('DROP TRIGGER IF EXISTS tr_sys_user_group_gravar_log_after_update');
        DB::unprepared('
            CREATE TRIGGER tr_sys_user_group_gravar_log_after_update AFTER UPDATE ON sys_user_group FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.user_id, "") != IFNULL(NEW.user_id, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_user_group", "update", "id", NEW.id, @client_addr, "user_id", OLD.user_id, 
                          NEW.user_id
                        );                    
                    END IF;
                    ...
                    IF (IFNULL(OLD.sys_group_id, "") != IFNULL(NEW.sys_group_id, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_user_group", "update", "id", NEW.id, @client_addr, "sys_group_id", OLD.sys_group_id, 
                          NEW.sys_group_id
                        );
                    END IF;
