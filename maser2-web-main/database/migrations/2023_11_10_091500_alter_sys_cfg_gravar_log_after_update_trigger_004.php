...
class AlterSysCfgGravarLogAfterUpdateTrigger004 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_sys_cfg_gravar_log_after_update AFTER UPDATE ON sys_cfg FOR EACH ROW
                BEGIN
					...
					IF (IFNULL(OLD.gerar_coletas_fixas, "") != IFNULL(NEW.gerar_coletas_fixas, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "update", "id", NEW.id, @client_addr, "gerar_coletas_fixas", 
                          OLD.gerar_coletas_fixas, NEW.gerar_coletas_fixas
                        ); 
					END IF;

					IF (IFNULL(OLD.dia_coletas_fixas, "") != IFNULL(NEW.dia_coletas_fixas, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "update", "id", NEW.id, @client_addr, "dia_coletas_fixas", 
                          OLD.dia_coletas_fixas, NEW.dia_coletas_fixas
                        ); 
					END IF;
					...
					IF (IFNULL(OLD.geo_lat_pavilion, "") != IFNULL(NEW.geo_lat_pavilion, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "update", "id", NEW.id, @client_addr, "geo_lat_pavilion", 
                          OLD.geo_lat_pavilion, NEW.geo_lat_pavilion
                        ); 
					END IF;

					IF (IFNULL(OLD.geo_lng_pavilion, "") != IFNULL(NEW.geo_lng_pavilion, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "sys_cfg", "update", "id", NEW.id, @client_addr, "geo_lng_pavilion", 
                          OLD.geo_lng_pavilion, NEW.geo_lng_pavilion
                        ); 
					END IF;
