...
class CreateColetaGravarLogAfterUpdateTrigger extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_coleta_gravar_log_after_update AFTER UPDATE ON coleta FOR EACH ROW
                BEGIN
                    ...
                    IF (IFNULL(OLD.tipo_carga, "") != IFNULL(NEW.tipo_carga, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "tipo_carga", OLD.tipo_carga, 
                          NEW.tipo_carga
                        );                    
                    END IF;                    
                    ...
                    IF (IFNULL(OLD.receber_nf_frete, "") != IFNULL(NEW.receber_nf_frete, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "colereceber_nf_freteta_id", OLD.receber_nf_frete, 
                          NEW.receber_nf_frete
                        );                    
                    END IF;

                    IF (IFNULL(OLD.km_inicial, "") != IFNULL(NEW.km_inicial, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "km_inicial", OLD.km_inicial, 
                          NEW.km_inicial
                        );                    
                    END IF;                    

                    IF (IFNULL(OLD.km_final, "") != IFNULL(NEW.km_final, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "km_final", OLD.km_final, 
                          NEW.km_final
                        );                    
                    END IF;                    

                    IF (IFNULL(OLD.geo_lat_coleta, "") != IFNULL(NEW.geo_lat_coleta, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "geo_lat_coleta", OLD.geo_lat_coleta, 
                          NEW.geo_lat_coleta
                        );                    
                    END IF;

                    IF (IFNULL(OLD.geo_lng_coleta, "") != IFNULL(NEW.geo_lng_coleta, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "geo_lng_coleta", OLD.geo_lng_coleta, 
                          NEW.geo_lng_coleta
                        );                    
                    END IF;                    

                    IF (IFNULL(OLD.geo_lat_entrega, "") != IFNULL(NEW.geo_lat_entrega, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "geo_lat_entrega", OLD.geo_lat_entrega, 
                          NEW.geo_lat_entrega
                        );                    
                    END IF;

                    IF (IFNULL(OLD.geo_lng_entrega, "") != IFNULL(NEW.geo_lng_entrega, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta", "update", "id", NEW.id, @client_addr, "geo_lng_entrega", OLD.geo_lng_entrega, 
                          NEW.geo_lng_entrega
                        );                    
                    END IF;
