...
class AlterVeiculoGravarLogAfterInsertTrigger001 extends Migration
{
    /**
     *
            ...
            CREATE TRIGGER tr_veiculo_gravar_log_after_insert AFTER INSERT ON veiculo FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.descricao, "") != "") THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "insert", "placa", NEW.placa, @client_addr, "descricao", NEW.descricao
                        );
                    END IF;                    
                    ...
                    IF (IFNULL(NEW.km_atual, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "insert", "placa", NEW.placa, @client_addr, "km_atual", 
                          NEW.km_atual
                        ); 
                    END IF;
