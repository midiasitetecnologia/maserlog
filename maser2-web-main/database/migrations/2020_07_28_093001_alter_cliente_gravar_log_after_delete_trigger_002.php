...
class AlterClienteGravarLogAfterDeleteTrigger002 extends Migration
{
    /**
     *
        ...
        DB::unprepared('
            CREATE TRIGGER tr_cliente_gravar_log_after_delete AFTER DELETE ON cliente FOR EACH ROW
                BEGIN                
                    ...
                    IF (IFNULL(OLD.local_distrib, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "cliente", "delete", "id", OLD.id, @client_addr, "local_distrib", OLD.local_distrib
                        ); 
                    END IF;

                    IF (IFNULL(OLD.solicitar_coletas, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value
                        ) 
                        VALUES 
                        ( @uuid, OLD.ass_user_id, @date_time, "cliente", "delete", "id", OLD.id, @client_addr, "solicitar_coletas", OLD.solicitar_coletas
                        ); 
                    END IF;
