...
class AlterVeiculoGravarLogAfterInsertTrigger004 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_veiculo_gravar_log_after_insert AFTER INSERT ON veiculo FOR EACH ROW
                BEGIN                                    
                    ...
                    IF (IFNULL(NEW.ignicao, "") != "") THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "veiculo", "insert", "placa", NEW.placa, @client_addr, "ignicao", 
                          NEW.ignicao
                        ); 
                    END IF;
