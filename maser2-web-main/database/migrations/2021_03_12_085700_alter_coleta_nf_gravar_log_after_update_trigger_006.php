...
class AlterColetaNfGravarLogAfterUpdateTrigger006 extends Migration
{
	/**
	 *
		...
		DB::unprepared('
            CREATE TRIGGER tr_coleta_nf_gravar_log_after_update AFTER UPDATE ON coleta_nf FOR EACH ROW
                BEGIN
					...
					IF (IFNULL(OLD.dig_cnpj, "") != IFNULL(NEW.dig_cnpj, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "dig_cnpj", OLD.dig_cnpj, 
                          NEW.dig_cnpj
                        );                    
                    END IF;
                    
                    IF (IFNULL(OLD.img_recibo, "") != IFNULL(NEW.img_recibo, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "img_recibo", OLD.img_recibo, 
                          NEW.img_recibo
                        );                    
					END IF;
					
					IF (IFNULL(OLD.solic_destino_id, "") != IFNULL(NEW.solic_destino_id, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "solic_destino_id", OLD.solic_destino_id, 
                          NEW.solic_destino_id
                        );                    
					END IF;

					IF (IFNULL(OLD.substituida, "") != IFNULL(NEW.substituida, "")) THEN
                        INSERT INTO user_log 
                        ( uuid, user_id, date_time, table_name, operation, pk, pk_value, client_addr, column_name, old_value, new_value
                        ) 
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "substituida", OLD.substituida, 
                          NEW.substituida
                        );                    
					END IF;
					...
					IF (IFNULL(OLD.observ, "") != IFNULL(NEW.observ, "")) THEN
                        ...
                        VALUES 
                        ( @uuid, NEW.ass_user_id, @date_time, "coleta_nf", "update", "id", NEW.id, @client_addr, "observ", OLD.observ, 
                          NEW.observ
                        );                    
                    END IF;
